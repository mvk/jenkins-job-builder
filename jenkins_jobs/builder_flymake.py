    ...
    if hasattr(obj, 'format'):
        if allow_empty:
            ...
            if result is not None:
                ...
                ret = CustomFormatter().format(obj, **paramdict)
        else:
            try:
                result = re.match('^{obj:(?P<key>\w+)}$', obj)
                if result is not None:
        ...
        for item in obj:
            if allow_empty:
                ret[CustomFormatter().format(item, **paramdict)] = \
    ...
    def expandYamlForTemplateJob(self, project, template, jobs_filter=None):
        ...
        for values in itertools.product(*dimensions):
            ...
            allow_empty_variables = self.config \
                ...
                and self.config.has_option('job_builder',
                                           'allow_empty_variables') \
                and self.config.get('job_builder', 'allow_empty_variables')
...
class Builder(object):
    ...
    def update_jobs(self, input_fn, names=None, output=None, n_workers=None):
        orig = time.time()
        ...
        if output:
            for job in jobs:
                ...
                # if not expect output to be a directory path
                output_dir = output
        ...
        if not jobs:
            logging.info("No jobs to update")
            return []
        ...
        # list
        if n_workers in (1, None) or len(p_params) == 1:
            results = [results]
    ...
    def update_job(self, input_fn, names=None, output=None):
        ...
        return self.update_jobs(input_fn, names, output)
